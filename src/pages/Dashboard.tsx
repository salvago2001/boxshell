...
import { Package, TrendingUp, Euro, ArchiveX, Box as BoxIcon } from 'lucide-react';
import {
...
export function Dashboard() {
  ...
  const { boxes, items, addBox, addItem, getStats, addToast, moveItem } = useStore();
  ...
  const handleDragEnd = (e: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      const targetBoxId = String(over.id).replace('box-', '');
      const item = items.find(i => i.id === String(active.id));
      if (item && item.boxId !== targetBoxId) {
        moveItem(String(active.id), targetBoxId);
        addToast(`Movido a ${boxes.find(b => b.id === targetBoxId)?.name ?? 'caja'}`, 'success');
      }
    }
  };
  ...
  return (
    <div className="min-h-screen bg-surface pb-24">
      <AppHeader
        subtitle={`${stats.totalBoxes} cajas · ${stats.totalItems} objetos`}
      ...
      >
              ...
              {boxesWithCount.length === 0 ? (
                ...
                <div className="space-y-2">
                  {boxesWithCount.map((box) => (
                    <DroppableBoxCard key={box.id} box={box} onClick={() => navigate(`/box/${box.id}`)} />
                  ))}
                </div>
              )}
            </Section>
        ...
        <DragOverlay>
          {draggingItem ? (
            <div className="opacity-90 shadow-xl rounded-xl scale-105">
              <ItemRow
                item={draggingItem}
                boxName={boxMap.get(draggingItem.boxId)?.name}
                onClick={() => {}}
              />
            </div>
          ) : null}
...
function DraggableItemRow({ item, boxName, onClick }: { item: Item; boxName?: string; onClick: () => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...listeners} {...attributes}
      ...
      <ItemRow item={item} boxName={boxName} onClick={onClick} />
    </div>
  );
}

function DroppableBoxCard({ box, onClick }: { box: Box & { itemCount: number; stockCount: number; soldCount: number }; onClick: () => void }) {
  const { isOver, setNodeRef } = useDroppable({ id: `box-${box.id}` });
  return (
    <div ref={setNodeRef} className={isOver ? 'ring-2 ring-brand rounded-xl' : ''}>
      <BoxCard box={box} onClick={onClick} />
    </div>
  );
}
